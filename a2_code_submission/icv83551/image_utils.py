...
def image_from_url(url):
    ...
    try:
        ...
        # Open with PIL to ensure the file handle is managed correctly
        with Image.open(fname) as pil_img:
            img = np.array(pil_img)

        # Now the file is closed, so Windows will allow the removal
        os.remove(fname)
        return img
        ...
        # Best practice: Clean up the file if it exists, even if an error occurred
